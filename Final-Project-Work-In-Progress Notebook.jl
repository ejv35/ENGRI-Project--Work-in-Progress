...
html"""
This design aims to optimize the production of 1,2 propanediol(PGDN) using the substrate maltose.
"""
...
# ╔═╡ 67b37cc9-5c79-43c3-838a-3ab7da13040a
html"""
...
html"""
<ul>
<li>Design Process will run 24/7/365.25</li>
...
html"""
Flux balance analysis is the mathematical approach to compute the needed flux for our reaction process. Our project team was given that the flux bounds were +/- 2.466 mmol per hour. With the given Flux Balance Analysis (FBA ) coding provided by Varner Labs, we input our given sugar feedstock and additional reactants and tested different inputs to find the optimal reaction rates. We determined that the maximum extent for our reaction was 2.466 mmol per hour and made the assumption that running the reactors at this maximum extent value was optimal (common-sensically). In addition, we were able to determine and check the input of Maltose, water, and KNO3 with the use of the stoichiometric coefficients of our reaction: all the stoiciometric coefficients in the reaction pathways used to convert Maltose to PGDN are 1, with the exception of:
<li>Maltose + H2O <=> 2 D-Glucose</li>
<li>Propane-1,2-diol + 2 KNO3 <=> PGDN +  2 KOH</li>
Thus, we determined the input rate such that all reagents would be completely consumed at the end of n reactors in series would be n*0.5*2.466 for Maltose and water and 2*2.466 to each individual reactor for KNO3 (resulting in 2*n*2.466 in total).

Through the use of the stoichiometric coefficients and number of reactor chips, were we able to determine the input flow rates for our reactants to attain the needed amount of PDGN.
"""

# ╔═╡ 1a75041f-a7b5-491d-a74e-7e4740f0ceed
html"""
Futhermore, we set the flow rate for our syring pump to 0.01 mL/hr in order to maximize the amount of time that our input molecule spends in the reactor. Although we assume that our molecules will fully react, this is still optimal to minimize the amount of water needed in the reactor - save the environment! 
"""
...
begin
	...
	# this is a dumb way to do this ... you're better than that JV come on ... NOTE: it's okay JV! It works! :)
...
html"""
Our team had the decision whether to run our microfluidic chips in parallel--in which equal feeds are fed into identical chips that operate in parallel and then recombined-- or in series-- in which the output of one reactor is the input of another. Using the provided Julia notebooks and coding for chips in parallel and in series, we determined that there were no significant changes in the output flow rates between both options. The code for these calculations is in separate Julia notebooks in the Github repository called SeriesChips.jl and ParallelChips.jl respectively.
"""

# ╔═╡ 5915ecf5-7353-4442-aa45-d226d8b60ca6
html"""
Another note about the chosen flow rate (F dot) for this reactor setup: when studying CSTRs earlier this semester, it was understood that reactor volume divided by flow rate was represented by Tau, which had units of time, and provided the average time spent by a single input molecule in the reactor before exiting. When Tau was large enough, it was assumed that the reaction within the reactor proceeded to completion, and that would be the reason for keeping the flow rate as small as possible if it wasn't assumed that reactions always proceeded to completion due to enzymatic catalysis and that extent of reaction and n dot weren't dependent on flow rate.
"""
...
html"""
Overall, this design is favorable: it produces revenue that much outweighs the cost invested in the product. Additionally, the yield is slightly above the goal target of 1.0g/hr and meets the 95% purity standard. When examining our Net Present Value analysis for our initial investment of $2880, it is evident that over 10 years our design process is extremely financially favorable when compared to a 10% annual investment rate.
"""
